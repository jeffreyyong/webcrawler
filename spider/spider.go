// Package spider implements a simple web crawler which maps a single site.
package spider
...
import (
	...
	"github.com/jeffreyyong/webcrawler/parser"
)
...
// Spider implemnets a simple web crawler. Should be instantiated with the
// new function.
type Spider struct {
	startURL *url.URL
	siteMap  *SiteMap
...
// New instantiates and returns a new Spider.
func New(u *url.URL, concur int) *Spider {
	siteMap := &SiteMap{Pages: make(map[string]parser.PageDetails)}
	idx := make(index)
	idx.add(u)
	return &Spider{u, siteMap, idx, concur}
}

// Crawl returns the SiteMap of website.
func (s *Spider) Crawl() *SiteMap {
	count := counter{}
	pagesToVisit := make(chan *url.URL, s.concur)
	results := make(chan namedPageDetails, s.concur)
	for i := 0; i < s.concur; i++ {
		go crawlPage(s.startURL, pagesToVisit, results)
	}
	...
	for {
		...
		case result := <-results:
			...
			s.siteMap.Pages[result.url.String()] = result.details
			count.decr()
		default:
			unvisitedLinks := s.idx.getUnvisitedLinks()
...
// URL query and fragement are stripped
func (i index) add(u *url.URL) {
	...
	if _, ok := i[u.String()]; !ok {
		...
		// Add the url to the index map, haven't visited yet.
		i[u.String()] = false
	}
}
...
func (i index) markVisited(u *url.URL) {
	u.Fragment = ""
	u.RawQuery = ""
	log.Printf("Marking %v as visisted", u)
	i[u.String()] = true
}
