...
// PageDetails decribes certain properties of an HTML page.
	...
	for {
		...
		case atom.A:
			...
			if resolvedURL.Host == pageURL.Host {
				details.InternalLinks = append(details.InternalLinks, resolvedURL.String())
			} else {
				details.ExternalLinks = append(details.ExternalLinks, resolvedURL.String())
			}
		}
	}
}
...
// If more than one attribute share the Key k, the value associated with
// the first attribute in the slice is retruned.
